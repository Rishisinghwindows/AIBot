...
struct ChatView: View {
    ...
    var body: some View {
                ...
                ZStack(alignment: .bottomTrailing) {
                    ...
                    if showScrollToBottom {
                        ScrollToBottomButton(action: {
                            // Scroll to bottom handled by ScrollViewReader
                        }, unreadCount: 0)
                        .padding(.trailing, 16)
                        .padding(.bottom, 16)
        ...
        // Inline navigation destination to avoid overlay hacks
        .background(
            NavigationLink(
                destination: MCPConfigView(
...
struct AnimatedDetailEmptyView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            withAnimation(.linear(duration: 2).repeatForever(autoreverses: false).delay(0.5)) {
                shimmerOffset = 300
            }

            // Animate particles
            for i in 0..<5 {
                withAnimation(.easeInOut(duration: 2).repeatForever(autoreverses: true).delay(Double(i) * 0.3)) {
                    particleOpacity[i] = Double.random(in: 0.3...0.7)
                }
            }
        }
    }
...
func toolIcon(for name: String) -> String {
    ...
    case "dosha_check": return "exclamationmark.triangle.fill"
    ...
    case "metro_info": return "tram.fill"
    // D23Bot News tool
...
func toolColor(for name: String) -> Color {
    ...
    case "dosha_check": return .red
    case "panchang": return .teal
    case "numerology": return .orange
    case "tarot": return .indigo
    // D23Bot Travel tools
    case "pnr_status", "train_status", "metro_info": return .blue
    // D23Bot News tool
...
func toolCategory(for name: String) -> String {
    switch name.lowercased() {
    case "weather", "web_crawl", "news": return "Web & Data"
    ...
    // D23Bot categories
    case "horoscope", "kundli", "kundli_matching", "dosha_check", "life_prediction", "panchang", "numerology", "tarot", "ask_astrologer": return "Astrology"
    case "pnr_status", "train_status", "metro_info": return "Travel"
...
struct ToolPickerView: View {
    ...
    private var groupedTools: [(String, [ToolInfo])] {
        let grouped = Dictionary(grouping: availableTools) { toolCategory(for: $0.name) }
        let order = ["General", "Astrology", "Travel", "Web & Data", "Documents & Database", "Google", "Atlassian", "Communication", "Development", "Custom", "Other"]
